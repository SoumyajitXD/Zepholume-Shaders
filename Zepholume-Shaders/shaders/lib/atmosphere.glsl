...
float zephAtmosphereFactor(float distance, float vertical, float rain) {
    ...
    float horizon = 1.0 - abs(vertical) * 0.16;
    return zephSaturate(curved * horizon * (0.82 + rain * 0.30));
#endif
}

# Hot-path optimisation, 0.1.0-dev
...
Texture sampling remains one essential source sample for textured geometry, water, clouds, and textured sky. No duplicate texture read was present to remove. There are no fragment `pow`, `normalize`, loops, or `discard` operations. Water trigonometry remains vertex-only and is compiled out in Ultra Lite.

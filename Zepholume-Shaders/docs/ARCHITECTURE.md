...
Structural validation checks 48 stages, includes, guards, pair varyings, profile matrices, option ranges, and one-colour-output policy. Generated source expands all declared profiles, dimensions, and two loader macro models. Standalone `glslangValidator` compiles that generated corpus, but neither it nor the mocked portability matrix validates Iris/Oculus patched source, a real GPU, or visual output.
